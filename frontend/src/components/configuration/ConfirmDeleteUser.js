...
export default function ConfirmDeleteUser({ open, userID, handleClose }) {
    ...
    function handleConfirm(){
        const requestOptions = {
            ...
            try{
                fetch(process.env.REACT_APP_API_BASE_URL + "/api/v1/users/id/"+userID, requestOptions)
                .then(response => {
                    if(response.ok){
                ...
                .catch(async (err) => {
                    ...
                    if(err === 403){
                      await refreshToken().then(()=>{
                        // new token stored in local storage
